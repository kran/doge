 ...
 */
public class H2FT extends FullText {
    ...
    public static void init(Connection conn) throws SQLException {
        try (Statement stat = conn.createStatement()) {
            ...
            String className = FullTextLuceneCJK.class.getName();
            ...
            stat.execute("CREATE ALIAS IF NOT EXISTS FTL_SEARCH_DATA_SORTED FOR '" + className + ".searchDataSorted'");
    ...
    public static ResultSet search(Connection conn, String text, int limit,
        ...
        return search(conn, text, limit, offset, false, null, null);
    }
    ...
    public static ResultSet searchData(Connection conn, String text, int limit,
                                       int offset) throws SQLException {
        return search(conn, text, limit, offset, true, null, null);
    }

    public static ResultSet searchDataSorted(Connection conn, String text, int limit,
                                       int offset, String sortFieldName, Boolean reverse) throws SQLException {
        return search(conn, text, limit, offset, true, sortFieldName, reverse);
    }
    ...
    protected static ResultSet search(Connection conn, String text,
                                      int limit, int offset, boolean data, String sortFieldName, Boolean reverse) throws SQLException {
...
//                Analyzer analyzer = access.writer.getAnalyzer();
                ...
                if(sortFieldName != null && !sortFieldName.isEmpty()) {
                    String[] fieldNameArray = sortFieldName.split(",");
                    SortField[] sortFields = new SortField[fieldNameArray.length];
                    for(int i = 0; i < fieldNameArray.length; i++) {
                        sortFields[i] = new SortedNumericSortField(fieldNameArray[i], SortField.Type.LONG, reverse);
                    }
                    Sort sort = new Sort(sortFields);
                    docs = searcher.search(query, maxResults, sort);
                } else {
    ...
    public static final class FullTextTrigger implements Trigger {
        ...
        public void init(Connection conn, String schemaName, String triggerName,
            ...
            columnTypeNames = new String[columnList.size()];
            ...
            for (int i = 0; rs.next(); i++) {
                ...
                columnTypeNames[i] = rs.getString("TYPE_NAME");
            }
            if (keyList.isEmpty()) {
        ...
        void insert(Object[] row, boolean commitIndex) throws SQLException {
            ...
            for (int i = 0, length = indexColumns.length; i < length; i++) {
                ...
                if(columnTypeNames[index].equals("JSON")) {
                    JSONValue value = JSONBytesSource.parse((byte[]) row[index], new JSONValueTarget());
                    List<Map.Entry<String, String>> flatMap = new ArrayList<>();
                    this.toFlatJson(value, columnName, flatMap);
                    for(Map.Entry<String, String> field : flatMap) {
                        doc.add(new Field(field.getKey(), field.getValue(), TextField.TYPE_NOT_STORED));
                        builder.append(" ");
                    ...
                    if(columnType == Types.BIGINT) {
                        doc.add(new NumericDocValuesField(columnName, (Long) row[index]));
                    }
                    else if (columnType == Types.TIMESTAMP) {
                        Timestamp data = null;
                        if(row[index] instanceof LocalDateTime) {
                            data = Timestamp.valueOf((LocalDateTime) row[index]);
                        }
                        else if(row[index] instanceof Timestamp) {
                            data = (Timestamp) row[index];
                        }

                        if(data != null) {
                            doc.add(new NumericDocValuesField(columnName,  data.getTime()));
                        }
                    } else if(columnType == Types.INTEGER) {
                        doc.add(new NumericDocValuesField(columnName, (int) row[index]));
                    }
                    else if(columnType == Types.REAL || columnType == Types.FLOAT) {
                        doc.add(new NumericDocValuesField(columnName, Float.floatToRawIntBits((float) row[index])));
                    }
                    else if(columnType == Types.DOUBLE) {
                        doc.add(new NumericDocValuesField(columnName, Double.doubleToLongBits((double) row[index])));
                    }
                    else if(columnType == Types.DECIMAL) {
                        var val = (BigDecimal) row[index];
                        doc.add(new NumericDocValuesField(columnName, Double.doubleToLongBits(val.doubleValue())));
                    }
                    else {
        ...
        private void toFlatJson(JSONValue value, String prefix, List<Map.Entry<String, String>> flatMap) {
            if(value == null) return;

            if(value instanceof JSONArray) {
                JSONArray curr = (JSONArray) value;
                for(var i = 0; i < curr.length(); i++) {
                    toFlatJson(curr.getElement(i), prefix, flatMap);
                }
            } else if (value instanceof JSONObject) {
                JSONObject curr = (JSONObject) value;
                Map.Entry<String, JSONValue>[] members = curr.getMembers();
                for (Map.Entry<String, JSONValue> member : members) {
                    toFlatJson(member.getValue(), prefix + "." + member.getKey(), flatMap);
                }
            } else {
                flatMap.add(Map.entry(prefix, value.toString()));
            }
        }
